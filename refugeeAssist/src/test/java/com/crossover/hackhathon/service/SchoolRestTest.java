...
public class SchoolRestTest {
    ...
    public void listSchool() throws Exception {
    			mockMvc.perform(get(RefugeeAssistConstants.BASE_URL+"school/"))
    ...
    public void getSchool() throws Exception {
    			this.setSchoolSuccess();
    			mockMvc.perform(get(RefugeeAssistConstants.BASE_URL+"school/"+idVal.toString()))
    			.andExpect(status().isOk())
    			.andExpect(content().contentType("application/json;charset=UTF-8"));
	}
    
    @Test
    public void updateSchoolSuccess() throws Exception {
    			
    	SchoolTransport vt = new SchoolTransport();
    	vt.setAddressId(1L);
    	vt.setName("Denizli University");
    	vt.setWebsite("www.denizliuni.edu.tr");
    	vt.setType(SchoolType.UNIVERSITY.toString());
    	
    	this.setSchoolSuccess();
    	mockMvc.perform(put(RefugeeAssistConstants.BASE_URL+"school/"+idVal.toString())
    ...
    @Test
    public void setSchoolSuccess() throws Exception{
    	SchoolTransport vt = new SchoolTransport();
    	vt.setAddressId(1L);
    	vt.setName("Istanbul University");
    	vt.setWebsite("www.istanbuluniversity.edu.tr");
    	vt.setType(SchoolType.UNIVERSITY.toString());
    	MvcResult result =mockMvc.perform(post(RefugeeAssistConstants.BASE_URL+"school/")
    	.content(this.json(vt))
		.contentType(RefugeeAssistConstants.contentType))
    	.andExpect(status().isOk())
    ...
    public void deleteSchool() throws Exception{
		this.setSchoolSuccess();
		mockMvc.perform(delete(RefugeeAssistConstants.BASE_URL+"school/"+idVal.toString()))
		.andExpect(status().isOk());
    }
